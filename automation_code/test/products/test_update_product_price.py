...
pytest_mark = [pytest.mark.products, pytest.mark.regression]
...
def test_update_regular_price_should_update_price():
    ...
    res_update = product_helper.call_update_product(product_id, payload=payload)

    # verify the response has the 'price' and 'regular_price' has updated and 'sale_price' is not updated
    assert res_update['price'] == new_price, f"Update product api call response. Updating the 'regular_price' did not " \
                                            f"update the 'price' field. price field actual value {res_update['price']}," \
                                            f"but expected: {new_price}"

    assert res_update['regular_price'] == new_price, f"Update product api call response. Updating the 'regular_price' did not " \
                                            f"update in the response. Actual response 'regular_price'={res_update['price']}," \
                                            f"but expected: {new_price}"
...
# TODO Failing ....Investigate   > check the dao select statement!!
@pytest.mark.tcid22
...
def test_update_on_sale_field_buy_updating_sale_price():
    ...
    # (tcid23) now update the 'sale_price' and verify the 'on_sale' is set to True
    ...
    # (tcid24) now update the sale_price to empty string and verify the 'on_sale is set to False
    product_helper.call_update_product(product_id, {'sale_price': ''})
    product_after_update = product_helper.call_retrieve_product(product_id)
